...
const pgTap = read(
  'supabase/tests/phase7_30b22b_ai_unlock_edge_browser_test.sql',
)
...
assert.match(
  migration,
  /totp_factor_mutation_enabled boolean not null default false/,
)
assert.match(
  migration,
...
assert.match(
  controlCleanup,
  /for update of transition skip locked[\s\S]*limit 500/,
)
assert.match(
  controlCleanup,
...
assert.doesNotMatch(
  migration,
  /grant (select|insert|update|delete) on private\./i,
)

assert.match(config, /\[functions\.admin-ai-unlock\][\s\S]*verify_jwt = true/)
assert.match(
  edge,
  /const FACTOR_ENTRY_ACTIONS[\s\S]*'authorizeTotpTransition'[\s\S]*'prepareTotpTransition'/,
)
assert.doesNotMatch(
...
assert.match(
  edge,
  /if \(action === 'verifyPin'\)[\s\S]*ADMIN_AI_NETWORK_PEPPER/,
)
assert.match(
  edge,
...
assert.match(
  edge,
  /action === 'preparePinMutation'[\s\S]*FOUR_DIGIT_PIN_PATTERN\.test\(body\.pin\)[\s\S]*service_unavailable/,
)
assert.match(
  edge,
  /action === 'setPin'[\s\S]*FOUR_DIGIT_PIN_PATTERN\.test\(body\.pin\)[\s\S]*service_unavailable/,
)
assert.match(
  edge,
  /action === 'completeBrowserEnrollment'[\s\S]*FOUR_DIGIT_PIN_PATTERN\.test\(body\.pin\)[\s\S]*pin_denied[\s\S]*service_unavailable/,
)
...
assert.match(
  browserCredential,
  /type RememberedBrowserIdentityScope[\s\S]*environmentId[\s\S]*membershipId[\s\S]*principalId/,
)
assert.match(browserCredential, /sameScope\(credential, scope\)/)
assert.match(
  browserCredential,
  /credential\.status === 'active'[\s\S]*Date\.parse\(credential\.expiresAt\) <= Date\.now\(\)[\s\S]*store\.delete\(credential\.id\)/,
)
assert.match(
  browserCredential,
  /withStore\('readwrite'[\s\S]*store\.getAll\(\)[\s\S]*store\.add\(credential\)/,
)
assert.match(
  browserCredential,
  /Date\.parse\(existing\.enrollmentExpiresAt\) > Date\.now\(\)[\s\S]*store\.delete\(existing\.id\)[\s\S]*store\.add\(credential\)/,
)
...
assert.match(
  activateBrowserEnrollment,
  /current\?\.status === 'active'[\s\S]*current\.expiresAt === expiresAt[\s\S]*sameCredentialProvenance\(current, expected\)[\s\S]*return current/,
)
assert.match(
  browserCredential,
  /samePendingEnrollment[\s\S]*store\.delete\(expected\.id\)/,
)
...
assert.match(
  aiPanel,
  /getRememberedBrowserEnrollmentStatus[\s\S]*remoteStatus\.status === 'active'[\s\S]*activatePendingRememberedBrowserEnrollment/,
)
assert.match(
  aiPanel,
  /credential\.enrollmentExpiresAt[\s\S]*createPendingRememberedBrowserEnrollment\(identityScope\)/,
)
assert.match(
  aiPanel,
  /confirmPendingBrowserEnrollmentWindow[\s\S]*beginRequestId/,
)
assert.doesNotMatch(
  aiPanel,
  /catch \(error\) \{[\s\S]{0,160}clearRememberedBrowserCredential\(credential\.id\)/,
)
...
assert.match(
  route,
  /isPhase730AdminTotpFactorMutationEnabled[\s\S]*AdminTotpFactorControlPanel/,
)
assert.match(route, /isPhase730AdminAiUnlockEnabled[\s\S]*AdminAiUnlockPanel/)
assert.match(
  route,
  /AdminAiUnlockPanel[\s\S]*environmentId: session\.environmentId[\s\S]*membershipId: session\.membershipId[\s\S]*principalId: session\.principalId/,
)
assert.ok(
...
assert.match(
  route,
  /transitionRecovery\.action === 'totp_factor_remove'[\s\S]*else \{[\s\S]*challengeAndVerify[\s\S]*finalizePersistedTotpFactorTransition/,
)
assert.match(
  route,
  /transitionCandidateCode[\s\S]*追加先の認証アプリに表示された6桁コード/,
)
assert.match(route, /期限切れ後の本人確認へ進む/)
assert.match(recovery, /finalizeRequestId: crypto\.randomUUID\(\)/)
assert.match(
  recovery,
  /lost success response must[\s\S]*recoverable after tab close/i,
)
...
assert.match(
  recovery,
  /error\.recoveryUnused[\s\S]*compareAndDeleteStoredRecovery\(recovery\)/,
)
assert.match(
  recovery,
...
assert.match(
  route,
  /purgeExpiredAdminTotpTransitionRecovery\(transitionRecoveryScope\)/,
)
...
assert.match(
  route,
  /getAdminTotpTransitionRecoveryScope\([\s\S]*data\.session\.user\.id[\s\S]*data\.session\.access_token/,
)
assert.match(factorPanel, /recoveryScope: AdminTotpTransitionRecoveryScope/)
assert.match(
  packageJson.scripts['test:e2e:phase7-30b22b-browser'],
  /desktop-chromium[\s\S]*desktop-webkit/,
)
assert.match(workflow, /npm run test:e2e:phase7-30b22b-browser/)
assert.match(browserE2e, /exportKey\('pkcs8', pending\.privateKey\)/)
assert.match(
  browserE2e,
  /Promise\.all\(\[create\(page\), create\(secondPage\)\]\)/,
)
assert.match(browserE2e, /scopeA[\s\S]*scopeB[\s\S]*otherTeacherAfter/)
assert.match(
  browserE2e,
  /recoveryScopeA[\s\S]*recoveryScopeB[\s\S]*restoreAdminTotpTransitionRecovery/,
)
assert.match(
  browserE2e,
...
assert.match(
  upgradeRunner,
  /--version'[\s\S]*20260809231342[\s\S]*phase7-30b22b-b22a-head-upgrade-probe\.sql[\s\S]*phase7-30b22b-b22a-head-upgrade-probe-test\.sql/,
)
assert.match(
  upgradeFixture,
...
assert.match(
  upgradeProbe,
  /without inferred trust binding[\s\S]*pending pre-B2\.2b enrollment is superseded without backfill[\s\S]*all B2\.2b source gates remain default OFF/,
)
assert.match(
  concurrency,
  /PHASE730B22B_TRANSITION_SLOT_READY[\s\S]*recordNullableServiceResult[\s\S]*transition-authorize-busy[\s\S]*status[\s\S]*available[\s\S]*transition-authorize-release/,
)
assert.match(
  concurrency,
...
assert.doesNotMatch(
  recovery,
  /transitionCandidateCode|totpCode|qrCode|cancelRequestId|appSessionReference/,
)
...
assert.match(
  factorPanel,
  /await finishFinalization\(\)[\s\S]*adminSupabase\.auth\.mfa\.unenroll/,
)
assert.match(
  factorPanel,
  /pending\.phase !== 'upstream_add'[\s\S]*await finishFinalization\(\)[\s\S]*challengeAndVerify/,
)
assert.match(
  factorPanel,
  /pending\.phase === 'control'[\s\S]*Scan this QR code before authorizing/,
)

const browserBeginHandler =
  edge.match(
