...
assert.match(managePolls, /DEFAULT_RECENT_POLL_LIMIT = 5/)
assert.match(managePolls, /\.eq\('status', 'open'\)/)
assert.match(managePolls, /\.neq\('status', 'open'\)/)
assert.match(managePolls, /const pollById = new Map<string, PollRow>\(\)/)
assert.match(managePolls, /const hasMore = recentPolls\.length > recentLimit/)
...
assert.match(materialManager, /admin_set_material_summary_publication/)
assert.match(materialManager, /target_actor_id: actorSessionId/)
