...
| Phase 7.30C2 unified Google Admin authorization, 20 Edge adapters and workspace | `test:phase7-30c2-static`, `test:phase7-30c2-ai-provider-static`, from-zero/all pgTAP, populated C1-head no-backfill/provider upgrade, generated types/DB lint, dual-transport/client credential contract and one exact-head CI run; paid provider calls remain excluded from this gate | Source/static/local candidate; exact-head DB/Edge/browser CI, Hosted/Human and activation remain HOLD   |
| Phase 7.30D owner Admin ledger, invitations and authority revocation            | `test:phase7-30d-static`, from-zero/all pgTAP, `test:phase7-30d-concurrency`, populated B1/C2-head invitation upgrade, generated types/DB lint, Local Edge and `test:e2e:phase7-30d-browser`; raw invitation token remains Edge-only and all gates default OFF                                          | Mocked Chromium/WebKit browser and source/static/non-live pass locally; database/Local Edge/exact-head CI, Hosted/Human and activation remain HOLD |
...
npm run test:phase7-30-upgrade          # populated 7.29/B1/B2/B2.2a/B2.2b/C1-head data through current C2 providers
...
## 5. What the 113 `test:*` scripts actually divide into

| Count | Kind                               | Where it runs                                                                                                                                                                                         |
| ----: | ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
|    73 | in `safeTestScripts`               | automatically inside `npm run test:ci:nonlive`; no need to invoke individually                                                                                                                        |
|    20 | needs the local Supabase stack     | invoked directly by CI's `local-supabase` job: 8 concurrency / lock-order suites, 5 `*-upgrade` suites, 3 local Edge suites and 4 local E2E entries                                                   |
