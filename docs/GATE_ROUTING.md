...
| Phase 7.30C2 unified Google Admin authorization and 16 Edge adapters          | `test:phase7-30c2-static`, `test:phase7-30c2-ai-provider-static`, from-zero/all pgTAP, populated C1-head no-backfill/provider upgrade, generated types/DB lint, dual-transport Edge contract and one exact-head CI run; the remaining four operations and workspace client stay HOLD | Source/static only; database, Local Edge, remaining C2, Hosted/Human and activation remain HOLD         |
