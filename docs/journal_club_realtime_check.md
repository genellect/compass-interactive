...
このチェックリストは、20人規模の研究室 Journal Club 前に、comments / likes / polls の最小動作を複数端末で確認するためのものです。

## 前提

- Supabase SQL Editorで `create_join_lecture_by_code_rpc.sql` が実行済み
- Supabase SQL Editorで `003_seed_journal_club_mvp.sql` が実行済み
- Supabase Realtimeで `comments` と `comment_likes` が有効
...
9. single choice pollに回答し、回答済み表示と結果更新を確認する。
...
9. Edgeでlike数がリロードなしで増えることを確認する。
10. Edgeで別participantとしてlikeし、Chrome側でもlike数が増えることを確認する。
...
6. スマホでlikeし、PC側のlike数が増えることを確認する。

## Poll確認

poll responsesのRealtimeはJC-2では未実装です。

- 回答直後、自分の画面ではlocal stateとRPC再取得で結果更新される。
- 他ブラウザのpoll resultsは自動更新されない場合がある。
- 他ブラウザでは `Poll結果再取得` ボタン、またはページreloadで結果を確認する。
- Journal Club MVPでは、poll resultsは回答後refreshまたは手動refreshで成立とする。
...
- Realtimeが届かない: Supabase Realtime publicationで `comments` / `comment_likes` が有効か確認する。
