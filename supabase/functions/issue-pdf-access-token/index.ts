...
Deno.serve(async (request) => {
  ...
  if (
    (body.action === 'admin' && (!hasGoogleCredential || hasDisplayToken)) ||
    (body.action === 'display' && (!hasDisplayToken || hasGoogleCredential)) ||
    (body.action === 'member' && (hasGoogleCredential || hasDisplayToken))
  ) {
    return jsonResponse(
      {
  ...
  if (body.action === 'admin' && !UUID_PATTERN.test(body.requestId ?? '')) {
  ...
  if (body.action === 'admin') {
    ...
    const { data: googleDisplayBindingData, error: googleDisplayBindingError } =
      await serviceClient.rpc('verify_and_claim_google_display_session_v1', {
    ...
    if (
      ...
      return jsonResponse(
        {
          credentialExpired: googleDisplayBinding?.recognized === true,
