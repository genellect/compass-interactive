...
CREATE TABLE public.phase5_upgrade_fixture (
...
GRANT SELECT, INSERT, UPDATE ON public.phase5_upgrade_fixture TO service_role;
...
BEGIN
  created_lecture_id := public.admin_create_lecture(
    'Phase 5 upgrade fixture',
    encode(extensions.digest(convert_to('P5-UPGRADE', 'UTF8'), 'sha256'), 'hex'),
    'P5-UPGRADE', null, null
  );
  PERFORM public.admin_set_lecture_status(created_lecture_id, 'start', null);
  PERFORM public.admin_register_pdf_document(
    created_lecture_id,
    'doc-upgrade', repeat('a', 64), 1, 'Upgrade PDF',
  ...
  PERFORM public.admin_finish_lecture_ai_operation(
    ...
    'succeeded',
    1000, 0, 500, 50,
    'provider-before-phase5', null
  );
  created_poll_id := public.admin_create_poll(
    created_lecture_id,
    'Existing teacher Poll survives the Phase 5 expansion',
    'single',
    ARRAY['Yes', 'No']
  );
  INSERT INTO public.phase5_upgrade_fixture (
    lecture_id,
    operation_id,
    poll_id,
    used_microusd,
