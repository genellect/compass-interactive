...
function mapSnapshot(raw: RawSnapshot, participantId: string | null): LiveSnapshot {
  const polls = raw.polls?.map<Poll>((poll) => ({
  ...
  const pollResults = raw.polls?.flatMap((poll) =>
  ...
  const pollResponses = raw.polls && participantId
    ? raw.polls
        .filter((poll) => poll.participant_option_ids.length > 0)
        .map<PollResponse>((poll) => ({
          createdAt: new Date().toISOString(),
          id: `snapshot-response-${poll.id}-${participantId}`,
  ...
  return {
    ...
    likeTotals: raw.like_totals?.map((total) => ({
...
export const supabaseLiveStateRepository = {
  async getSnapshot({
    ...
    const { data, error } = await supabase.rpc('get_lecture_live_snapshot', {
      ...
      target_participant_id: participantId ?? undefined,
    })
    ...
    return mapSnapshot(data as unknown as RawSnapshot, participantId)
  },
}
